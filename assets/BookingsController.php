...
@copyright
Fleet Manager v6.6
Copyright (C) 2017-2024 Hyvikk Solutions <https://hyvikk.com/> All rights reserved.
		...
		if ($request->ajax()) {
			...
			return DataTables::eloquent($bookings)
				...
				->addColumn('return_booking', function ($row) {
					...
					if($row->getMeta('return') == 1)
					{
						$b=Bookings::where('id',$row->getMeta('parent_booking_id'))->first();

						if(isset($b))
						{
							return url('/assets/customer_dashboard/assets/img/return_way5.svg');
						}
						else
						{
							return url('/assets/customer_dashboard/assets/img/one_way5.svg');
						}
					}
					else
					{
	...
	public function destroy(Request $request)
{
	...
	public function store(BookingRequest $request) {
		...
		if ($xx) {
			if ($request->get("travellers") > $max_seats) {
				...
				$key = (Hyvikk::api('api_key') ?? 'AIzaSyAfvqtQFHwnAu6fma1-094SYd9l8o4uMSw');
				...
				if(isset($request->booking_type) && $request->booking_type  == "return_way")
				{
					...
					$key2 = (Hyvikk::api('api_key') ?? 'AIzaSyAfvqtQFHwnAu6fma1-094SYd9l8o4uMSw');
	...
	public function update(BookingRequest $request) {
		...
		if(isset($request->booking_type) && $request->booking_type == "return_way")
		{
			...
			$max_seats1 = VehicleModel::find($request->get('return_vehicle_id'))->types->seats;
			...
			$key1 = (Hyvikk::api('api_key') ?? 'AIzaSyAfvqtQFHwnAu6fma1-094SYd9l8o4uMSw');
			...
			if ($dataFetch1['status'] === 'OK') {
				...
				$total_kms1=explode(" ", str_replace(",", "", $dataFetch['routes'][0]['legs'][0]['distance']['text']))[0];
